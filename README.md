...
![하동미차 대표 이미지](./assets/images/hadong-mist-tea-keyvisual-v2.png)

> 대표 이미지는 최종 영상의 마지막 프레임을 바탕으로 사후 생성한 프로젝트 커버입니다. 영상 생성 과정의 이미지 시드나 레퍼런스로 사용하지 않았습니다.
...
| 대표 이미지 | OpenAI ImageGen / 최종 영상 프레임 기반 사후 생성 |
| 영상 편집 도구 | Shotcut |
...
| --- | --- | --- |
| 이미지 | OpenAI ImageGen | 최종 영상의 마지막 프레임을 바탕으로 선명한 GitHub 대표 이미지 생성. 영상 제작의 입력 이미지로는 사용하지 않음 |
...
| 음성 합성 | 네이토 TTS 실험 후 최종본 제외 | 단일 입력창이 연출 지시와 실제 대사를 구분하지 않고 전체 문장을 발화해 원하는 길이와 톤을 얻지 못함. 기존 환경음과의 추가 믹싱 부담도 있어 네이티브 음성을 유지 |
| 통합 편집 | Shotcut | 생성된 씬 연결, 최종 길이 조정 및 출력 파일 인코딩 |

도구 접근 제한에 대비한 대안은 이미지 생성 도구, 비디오 생성 `Veo 3.1`, 음성 생성 `TTS-1 HD`로 설정했습니다. Veo 3.1은 실제 비교본 제작에 사용했고, 네이토 환경의 TTS도 별도로 시험했습니다. 이미지 생성은 영상 완성 후 OpenAI ImageGen으로 대표 이미지를 만드는 데 사용했으며, 영상의 이미지 시드나 레퍼런스로는 사용하지 않았습니다.

### 별도 TTS를 최종본에 사용하지 않은 이유

네이토의 TTS 입력창에 목소리 연출 지시와 실제 대사 `안개가 키운 잎`을 하나의 프롬프트로 입력해 시험했습니다. 그러나 이 환경에서는 세부 지시를 음성 스타일 명령으로 해석하지 않고 입력문 전체를 발화 대사로 처리했습니다. 그 결과 한 문장만 읽어야 하는 음성이 지시문까지 모두 낭독한 27.408초 오디오로 생성됐습니다.

- [TTS 실험 오디오 듣기](./evidence/tts-test/tts-instruction-readout.mp3)
- [TTS 생성 화면 보기](./evidence/tts-test/tts-instruction-readout.png)
- [TTS 실험 기록](./evidence/tts-test/README.md)

대사만 입력하면 길이는 줄일 수 있지만, 같은 입력창에서는 원하는 성별·온도·속도·강세 같은 세부 연출을 함께 지정하기 어려웠습니다. 또한 Shotcut을 장면 자르기와 연결 중심으로 사용한 상황에서, Sora 영상에 이미 포함된 환경음 위에 별도 TTS 음성을 자연스럽게 배치하려면 구간별 음량·타이밍·페이드와 믹싱을 추가로 조정해야 했습니다. 따라서 TTS 결과는 실험 자료로만 보존하고 최종본에는 Sora의 네이티브 오디오를 사용했습니다.
...
- 영상 생성에는 이미지 시드나 이미지 레퍼런스를 사용하지 않았습니다. 이미지 생성 AI는 최종 영상 프레임을 바탕으로 GitHub 대표 이미지를 사후 제작하는 데만 사용했습니다.
- 오디오는 Sora 2 Pro의 네이티브 생성 기능을 사용했으며, Veo 3.1 비교본에서는 오디오 트랙이 있었지만 요청한 사람 음성이 생성되지 않았습니다.
- 네이토 TTS는 연출 지시와 대사를 하나의 입력창에 넣었을 때 전체 프롬프트를 발화해, 짧은 광고에 사용할 수 없는 27.408초 음성이 생성됐습니다.
...
- 네이토 TTS 실험에서는 세부 연출 지시가 명령으로 적용되지 않고 모두 대사로 처리됐습니다. 대사만 입력할 경우 스타일 통제가 어렵고, 컷 자르기와 연결 중심의 편집 범위에서 기존 환경음과 별도 음성을 자연스럽게 믹싱하기도 어려워 화면과 함께 생성된 네이티브 오디오가 더 적합했습니다.
...
| 이미지 생성 AI 사용 | 충족 | OpenAI ImageGen으로 최종 영상 프레임 기반 대표 이미지 생성. 영상 레퍼런스로는 미사용 |
...
| 별도 TTS 비교 실험 | 충족 | 네이토 TTS의 단일 입력창에서 연출 지시까지 발화되는 27.408초 결과를 확인하고 최종본에서 제외 |
...
│   └── images/
│       ├── README.md
│       ├── hadong-mist-tea-keyvisual.png
│       └── hadong-mist-tea-keyvisual-v2.png
├── evidence/
│   └── tts-test/
│       ├── README.md
│       ├── tts-instruction-readout.mp3
│       └── tts-instruction-readout.png
└── videos/
    ├── README.md
    ├── 1-1.mp4 ... 1-4.mp4
    ├── 2-1.mp4 ... 2-4.mp4
