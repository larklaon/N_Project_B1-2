...
각 씬을 약 4.09초 길이로 3개씩 생성해 총 9개의 후보를 비교했습니다. 선택한 장면을 Shotcut에서 연결하고 메시지 전달에 불필요한 앞뒤 구간을 제거해 최종본을 만들었습니다.
...
flowchart LR
    ...
    D --> E["비주얼과 네이티브 오디오 동시 비교"]
    E --> F["안정성형·완성도형 프롬프트 비교"]
    F --> G["씬별 결과 선택"]
    G --> H["Shotcut 컷 편집 및 H.264·AAC 출력"]
```

영상 생성 전에 브랜드 메시지와 장면의 기능을 먼저 결정했습니다. 별도의 이미지 키프레임은 만들지 않았으며, 각 씬이 앞 장면의 물체를 그대로 복제하는 대신 다음 생산 단계로 자연스럽게 이어지도록 설계했습니다. 각 단계는 동일 프롬프트 계열로 세 번 생성해 구도, 움직임, 글자 표현과 오디오를 비교했습니다. 오디오는 별도 TTS나 음악 생성 도구를 결합하지 않고 Sora 2 Pro에서 화면과 함께 생성해 장면과 소리의 타이밍 불일치를 줄였습니다.
...
| 결과 파일명 | [`1-1.mp4`](./videos/1-1.mp4), [`1-2.mp4`](./videos/1-2.mp4), [`1-3.mp4`](./videos/1-3.mp4) |
...
| 결과 파일명 | [`2-1.mp4`](./videos/2-1.mp4), [`2-2.mp4`](./videos/2-2.mp4), [`2-3.mp4`](./videos/2-3.mp4) |
...
| 결과 파일명 | [`3-1.mp4`](./videos/3-1.mp4), [`3-2.mp4`](./videos/3-2.mp4), [`3-3.mp4`](./videos/3-3.mp4) |
...
씬 번호 뒤의 두 번째 숫자는 동일한 목적과 프롬프트 계열로 생성한 후보 번호입니다. 모든 후보와 최종본은 `videos/` 폴더에 보관합니다.
