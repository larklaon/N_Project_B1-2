...
하동의 차밭에서 시작해 찻잎의 가공 과정을 지나 한 잔의 차로 완성되는 흐름을 담은 생성형 AI 브랜드 광고 프로젝트입니다. 이미지 레퍼런스나 별도의 음성 합성 없이, **Sora 2 Pro의 텍스트 기반 영상 생성과 네이티브 오디오 생성**을 사용했습니다.
...
| 화면비 / 해상도 | 16:9 / 1280×720 이상 |

## 영상

최종 영상은 [`videos/`](./videos/) 폴더에 추가합니다.

- 최종 제출 파일명: `hadong-mist-tea-final.mp4`
- 권장 제출 사양: 1280×720 이상, 24~30fps, H.264 비디오, AAC 오디오
- 원본 생성 구조: 4초 × 3개 씬
- 제출 전 확인: 미션의 `10초 이내` 기준을 엄격히 적용할 경우, 현재 12초 구성은 10초 이하로 재생성하거나 허용 여부를 확인해야 합니다.
...
flowchart LR
    ...
    B --> C["공통 스타일 키워드 고정"]
    C --> D["Sora 2 Pro 텍스트 투 비디오 생성"]
    D --> E["비주얼과 네이티브 오디오 동시 검토"]
    E --> F["안정성형·완성도형 프롬프트 비교"]
    F --> G["최종 결과 선택"]
```

영상 생성 전에 브랜드 메시지와 장면의 기능을 먼저 결정했습니다. 별도의 이미지 키프레임은 만들지 않았으며, 각 씬이 앞 장면의 물체를 그대로 복제하는 대신 다음 생산 단계로 자연스럽게 이어지도록 설계했습니다. 오디오 역시 별도 TTS나 음악 생성 도구를 결합하지 않고 Sora 2 Pro에서 화면과 함께 생성해 장면과 소리의 타이밍 불일치를 줄였습니다.
...
| 비디오 | Sora 2 Pro | 산지, 가공, 제품 히어로샷을 텍스트만으로 시네마틱하게 생성 |
| 오디오 | Sora 2 Pro 네이티브 오디오 | 영상과 동시에 환경음, 가공음, 차 따르는 소리 및 음악 분위기를 생성해 동기화 유지 |
| 음성 합성 | 별도 TTS 미사용 | 영상 생성 단계에서 필요한 오디오까지 함께 생성되어 별도의 음성 합성과 합성 편집을 생략 |
| 통합 편집 | 미사용 | 핵심 비주얼과 오디오가 결합된 결과를 생성 단계에서 직접 확보 |

도구 접근 제한에 대비한 대안은 이미지 생성 `GPT Image 2`, 비디오 생성 `Veo 3.1`, 음성 생성 `TTS-1 HD`로 설정했습니다. 이 도구들은 실제 최종본 생성에는 사용하지 않았습니다.
...
| 사용 도구 / 목적 | Sora 2 Pro: 텍스트 기반 비디오와 산바람·새소리·잔잔한 음악을 함께 생성 |
| 출력 결과 요약 | 지역의 청정함과 차분한 브랜드 분위기를 한 번에 보여주는 도입부를 확보했다. |
| 결과 파일명 | `hadong-mist-tea-scene-01.mp4` |
...
| 사용 도구 / 목적 | Sora 2 Pro: 가공 과정의 움직임과 잎 스치는 소리·덖음 소리를 동시에 생성 |
| 출력 결과 요약 | 찻잎의 형태와 색이 달라지는 모습을 장인성과 시간의 표현으로 활용했다. |
| 결과 파일명 | `hadong-mist-tea-scene-02.mp4` |
...
| 사용 도구 / 목적 | Sora 2 Pro: 제품 히어로숏, 브랜드 텍스트, 차 따르는 소리, 음악의 마무리를 함께 생성 |
| 출력 결과 요약 | 일부 글자 왜곡이 있었지만 조명·패키지·카피가 결합되면서 가장 높은 광고적 완성도를 얻었다. |
| 결과 파일명 | `hadong-mist-tea-scene-03.mp4` |
...
- 이미지 생성 AI와 별도 오디오 생성 AI를 각각 사용하라는 기능 요구사항은 충족하지 않았습니다. 대신 Sora 2 Pro 하나로 영상과 오디오를 동시에 생성한 단일 도구 파이프라인을 실험했습니다.
- 이미지 시드나 스타일 레퍼런스를 사용할 수 없어 각 씬의 세부 사물과 구도는 완전히 동일하지 않습니다.
- 생성형 영상의 한글 및 제품 라벨 표현에는 일부 왜곡이 남았습니다.
- 4초 단위의 세 장면은 총 12초이므로, `10초 이내` 제출 규격을 엄격하게 적용하면 길이 조정이 필요합니다.
- 별도의 편집 도구를 사용하지 않아 컷 길이, 자막, 색보정, 오디오 레벨을 후반 단계에서 미세 조정하지 않았습니다.
...
- 별도 TTS는 발음과 목소리 통제가 필요할 때 유리하지만, 이를 영상과 결합하려면 추가 편집 단계가 필요합니다. 이번 작업에서는 네이티브 오디오가 더 단순하고 자연스러운 선택이었습니다.
...
| 비디오 생성 AI 사용 | 충족 | Sora 2 Pro 사용 |
| 오디오 생성 AI 별도 사용 | 부분 충족 | 별도 도구가 아닌 Sora 2 Pro 네이티브 오디오 사용 |
| 마지막 브랜드 인지 장치 | 충족 | Scene 03에 브랜드명, 패키지, 슬로건 포함 |
| 10초 이내 최종 영상 | 확인 필요 | 원본 구성은 4초 × 3씬, 총 12초 |
...
└── videos/
    ├── README.md
    ├── hadong-mist-tea-scene-01.mp4
    ├── hadong-mist-tea-scene-02.mp4
    ├── hadong-mist-tea-scene-03.mp4
    └── hadong-mist-tea-final.mp4
```

현재 영상 파일은 저장소에 포함되지 않았으며, 생성 결과를 준비한 뒤 위 규칙에 맞춰 `videos/` 폴더에 추가합니다.
